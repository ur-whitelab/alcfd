...
def check_laminar(pipe_D, inlet_v, rho=1.225, muo=1.7894e-5):
    if not isinstance(pipe_D, (list, tuple)):
        pipe_D = np.array(pipe_D)
        inlet_v = np.array(inlet_v)
    re_number = rho*inlet_v*pipe_D/muo
    a = np.where(re_number > 2100)[0]
    if len(a) != 0:
        print(f'Model input {a} parameters will result a turbulent flow, Re = {re_number[a]}. These parameters will be dropped.')
    # assert len(
